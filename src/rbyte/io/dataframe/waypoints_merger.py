...
class DataFrameWaypointsMerger:
    ...
    def __init__(
        ...
        assert coordinates_type in ["lat_lon", "xy"], "coordinates_type must be either 'lat_lon' or 'xy'"
    ...
    def __call__(self, input: pl.DataFrame) -> pl.DataFrame:
        ...
        if self.predict_mode:
            ...
            df = df.with_columns(
                ...
                .map_elements(
                    lambda row: list(
                        zip(row[self.wpts_lat_y_col], row[self.wpts_lon_x_col], strict=False)
                    ),
                    return_dtype=pl.List(pl.List(pl.Float64)),
                )
                .alias(self.wpts_lat_lon_col)
            ).drop(self.wpts_lat_y_col, self.wpts_lon_x_col)
        ...
        return df[:-1].drop(self.wpts_lat_y_col, self.wpts_lon_x_col) # hack to handle last frame rbyte problem

    def _build_df_wpts(self, waypoints_path: os.PathLike) -> pl.DataFrame:
        df_wpts = self._json_to_df(waypoints_path)
        df_wpts = pl.concat([df_wpts] + [df_wpts[-1, :]] * (self.n_wpts - 1)) # duplicate last waypoints
    ...
    def _json_to_df(self, file_path: os.PathLike) -> pl.DataFrame:
        ...
        data = {self.timestamp_col: [], self.heading_col: [], self.wpts_lon_x_col: [], self.wpts_lat_y_col: []}
        ...
        return (
            pl.DataFrame(data)
            .with_columns(
                pl.from_epoch(pl.col(self.timestamp_col) * 1e9, time_unit="ns").alias(self.timestamp_col)
            )
    ...
    def _center_and_rotate(
        ...
        x_wpts, y_wpts = self.transform(
            df[self.wpts_lon_x_col].explode(), 
            df[self.wpts_lat_y_col].explode()
        )
        wpts_series = gpd.GeoSeries(shapely.points(x_wpts, y_wpts))

        # ego
        x_ego, y_ego = self.transform(
            df.select(pl.col(self.ego_lon_x_col).repeat_by(self.n_wpts).flatten().explode()), 
            df.select(pl.col(self.ego_lat_y_col).repeat_by(self.n_wpts).flatten().explode())
        )
        ...
        headings = df.select(pl.col(self.heading_col).repeat_by(self.n_wpts).flatten().explode())
